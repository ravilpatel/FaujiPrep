...
function randomFile(excludeSet) {
  const options = audioFiles.filter((file) => !excludeSet.has(file));
  return options[Math.floor(Math.random() * options.length)];
}

function pickStimulus() {
  if (state.history.length >= 2 && Math.random() < config.matchChance) {
    return { file: state.history[state.history.length - 2], isMatch: true };
  }
  const exclude = new Set();
  const lastTwo = state.history.slice(-2);
  lastTwo.forEach((file) => exclude.add(file));
  return { file: randomFile(exclude), isMatch: false };
}

function playTone(type) {
  if (!state.soundOn) return;
  const ctx = window.__audioReflex || new (window.AudioContext || window.webkitAudioContext)();
  window.__audioReflex = ctx;
...
async function runStimulusRound() {
  ...
  setStatus('Listen and compare 2-back.');
  playStimulus(stimulus.file);
  ...
  await pauseableWait(config.minGap);
}
...
function getInsight(accuracy, attentionIndex, falsePos) {
  if (accuracy >= 0.85 && attentionIndex >= 80 && falsePos <= 2) {
    return {
      title: 'Excellent Auditory Working Memory',
      detail: 'High accuracy and controlled responses under auditory load.',
    };
  }
