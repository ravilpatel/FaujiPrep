...
function applyStimulus(shape) {
  ui.stimulus.classList.toggle('circle', shape === 'circle');
}

function playTone(type) {
  if (!state.soundOn) return;
  const ctx = window.__shapeAudio || new (window.AudioContext || window.webkitAudioContext)();
  window.__shapeAudio = ctx;
...
async function runStimulusRound() {
  ...
  state.currentRule = shapeRules[shape];
  ...
  setStatus(shape === 'circle' ? 'Circle: Press F' : 'Square: Press J');
...
function getInsight(accuracy, attentionIndex, falsePos) {
  if (accuracy >= 0.9 && attentionIndex >= 85 && falsePos <= 2) {
    return {
      title: 'Excellent Shape Discrimination',
      detail: 'Strong recognition speed with disciplined responses. Maintain this standard.',
    };
  }
  if (accuracy >= 0.75 && attentionIndex >= 70) {
    return {
      title: 'Good Reflex Accuracy',
      detail: 'Consistent responses. Reduce misses to reach elite performance.',
    };
  }
  if (falsePos >= 4) {
    return {
      title: 'Impulsive Response Pattern',
      detail: 'Slow slightly and confirm the shape before responding.',
    };
  }
  return {
